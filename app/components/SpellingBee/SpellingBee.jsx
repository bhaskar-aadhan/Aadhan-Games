...
import { getUserLevelData } from '../../services/spellingbee';
...
const SpellingBee = ({ letters }) => {
    ...
    const handleWordValidation = () => {
        const validWords = ['MSLBD', 'MBBS', 'GLSTM']
        ...
        <div className='w-full h-full'>
            <div className='w-full h-full flex flex-col items-center justify-center gap-[3rem]'>
                <div className='w-[90%] mx-auto flex flex-col justify-center items-start gap-2'>
                    <div className='w-full spellingbee_progress border-t-[0.5px] border-t-solid border-t-[#d3d3d3] py-1'>
                        <ScoreProgress UserLevelData={UserLevelData} score={score} />
                    </div>
                    <div className="w-full h-[40px] spellingbee_correctwords text-[grey] text-[14px] font-medium flex justify-start items-center border-y-[0.5px] border-y-solid border-y-[#d3d3d3]">
                        <div className='flex-1 font_poppins'>{correctWords?.length === 0 ? "Your Words..." : correctWords?.join(", ")?.toLowerCase()}</div>
                        <div>
